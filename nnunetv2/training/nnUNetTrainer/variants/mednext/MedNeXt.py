...
from nnunetv2.training.nnUNetTrainer.variants.mednext.blocks import *
...
class MedNeXt(nn.Module):
    ...
    def __init__(self, 
        ...
        checkpoint_style: str = None,             # Either inside block or outside block
        checkpoint_lobo: bool = False,            # Checkpoint leave one block out
    ...
    def iterative_checkpoint(self, sequential_block, x):
        ...
        """
        for l_idx, l in enumerate(sequential_block):
            if l_idx==0 and self.leave_one_block_out:
                x = l(x)
                continue
            x = checkpoint.checkpoint(l, x, use_reentrant=False)
        return x
